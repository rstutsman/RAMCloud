...
class LoadBalancer {
  public:
    explicit LoadBalancer(const char* coordinatorLocator)
        : ramcloud{coordinatorLocator}
        , dispatchDelay{}
        , readRate{}
        , stats()
...
try
{
    ...
    po::options_description desc{
        "Usage: LoadBalancer [options]\n\n"
        "Collects statistics from across a RAMCloud cluster, determines when\n"
        "and why servers are overloaded, and initiates migrations to rectify\n"
        "performance issues.\n\n"
        "Allowed options:"};
    ...
    LoadBalancer balancer{coordinatorLocator.c_str()};
    balancer.run();
